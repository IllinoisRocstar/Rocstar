...
IntList::IntList( const IntList& olist) :
  d_size(olist.d_size) {

  // just add all the list
  Elem* oelem = olist.d_first;
  Elem* prev(0);
  while( oelem ){
    Elem *nelem = new IntList::Elem( oelem->d_val );
    if( prev ){
      prev->d_next = nelem;
    }
    else {
...
const IntList& IntList::operator=( const IntList& olist){
...
void IntList::append(int val){
  ...
  if( bef ){
    bef->d_next = new IntList::Elem( val );
  }
  else {
...
boolean IntList::move_to(int val){
  d_current = d_first;
