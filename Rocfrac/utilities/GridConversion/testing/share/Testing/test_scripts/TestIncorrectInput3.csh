...
set InputDir=TestIncorrectInput3
set Outputs=screen.err
            #ENTER EXEC OUTPUT FILE NAME(S) HERE
            #IF ONE USE FORMAT Outputs=Filename
            #IF MULTIPLE USE FORMAT Outputs=(Filename1 Filename2 Filename3 ...)
set OutputsCheck=screenCheck.err
            #ENTER SAVED OUTPUT FILE TO CHECK AGAINST HERE
...
set TestName=TestIncorrectInput3
...
($3/gridConversion incorrect3.grd bcs.inp -v 2 -o out.dat > screen.dat) >& screen.err
...
#(This uses our own speical diff (diffdatafiles) from IRAD
#that can compare numbers with a tolerance. See documentation
#with it for more information on how to use it.)
@ i = 1
foreach file (${Outputs})
  $4/diffdatafiles ${file} $OutputsCheck[$i] -t 1.0e-10
  if($? != 0) then
