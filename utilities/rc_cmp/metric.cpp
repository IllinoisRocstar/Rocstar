...
metric::metric(std::vector<points*> datasets1, std::vector<points*> datasets2, 
               int var_ds1, int var_ds2,
               ofstream &outfile, string outprefix) : outfile(outfile)
{

		this ->restrictToBoxRange = false;

    this->outprefix = outprefix;

    this->datasets1 = datasets1;
...
void metric::set_datasets1(std::vector<points*> datasets1){
    this->datasets1 = datasets1;
}

void metric::set_datasets2(std::vector<points*> datasets2){
    this->datasets2 = datasets2;
}
...
/* mean square error -----------------------------------------------------*/
/*
mse::mse(std::vector<points*> datasets1, std::vector<points*> datasets2, 
         int var_ds1, int var_ds2,
         ofstream &outfile, string outprefix)
         : metric(datasets1, datasets2, var_ds1, var_ds2, outfile, outprefix){
...
long double mse::compare(){
    ...
    for(int i=0; i<dataset1->get_num_points(); i++)
    {
        ...
        if( this ->restrictToBoxRange &&
        		this ->range ->hasPoint( p1.vals[0], p1.vals[1], 0.0 ) &&
        		this ->range ->hasPoint( p2.vals[0], p2.vals[1], 0.0 ) )
        {
        ...
        else if( ! this ->restrictToBoxRange )
        {
...
/*
rmse::rmse(std::vector<points*> datasets1, points *dataset2, int var_ds1, int var_ds2,
            ofstream &outfile, string outprefix)
            : metric(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
/*
scc::scc(std::vector<points*> datasets1, points *dataset2, int var_ds1, int var_ds2,
            ofstream &outfile, string outprefix)
            : metric(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
/* map -------------------------------------------------------------------*/
/*
map::map(std::vector<points*> datasets1, points *dataset2, int var_ds1, int var_ds2,
            ofstream &outfile, string outprefix)
            : metric(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
rgb_color map::get_color_single_sided(long double pt_val, long double range, long double min){
    ...
    rgb_color ret;
    ret.r = gHeatedObject[val][0];
    ret.g = gHeatedObject[val][1];
    ret.b = gHeatedObject[val][2];
...
rgb_color map::get_color_double_sided(long double pt_val, long double range, long double min,
    ...
    if( mid_point >= min && mid_point <= (min+range) ){
        ...
        rgb_color ret;
        ret.r = gBlueToCyan[gc_val][0];
        ret.g = gBlueToCyan[gc_val][1];
        ret.b = gBlueToCyan[gc_val][2];
...
/* difference map --------------------------------------------------------*/
/*
difmap::difmap(std::vector<indexed_points*> datasets1, indexed_points *dataset2, int var_ds1, int var_ds2,
                ofstream &outfile, string outprefix)
    : map(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
string difmap::compare(){
    ...
    //Difference map requires indexed data
    //Try to convert the datasets to indexed_points datasets
    ...
    indexed_points* write_points = ds2_ip->clone();

    for(int x=0; x<ds1_ip->get_dim()[0]; x++)
    {
        for(int y=0; y<ds1_ip->get_dim()[1]; y++)
        {
            for(int z=0; z<ds1_ip->get_dim()[2]; z++)
            {
                ...
                //TODO: loop over every dep_var
								...
								pnt p1 = ds1_ip ->get_indexed_point( loc );
								pnt p2 = ds2_ip ->get_indexed_point( loc );
										...
										this ->range ->hasPoint( p2.vals[0],p2.vals[1],0.0 				) )
								{
									...
									//Get the difference at each point
									long double pt_val = fabs( ds1_ip->get_indexed_point(loc).vals[var_ds1]
																						 - ds2_ip->get_indexed_point(loc).vals[var_ds2] );

									int index =   x * write_points->get_dim()[1]
															+ y * write_points->get_dim()[2]
															+ z;

									//Store difference to dataset
									write_points->set_point_val(index, var_ds1, pt_val);
								}
								else if( ! this ->restrictToBoxRange )
								{
									std::vector<int> loc;
    ...
    TecplotOrderedWriter t_writer;
    t_writer.setOutputFilename(output_name);

    if(!t_writer.init())
        return string("Output file could not be opened");

    //Create variable names
    std::vector<string> variables;
    variables.push_back("Dep1");
    variables.push_back("Dep2");
    variables.push_back("Comparison");

    t_writer.addOrderedPartition(string("Partition 1"), write_points, var_ds1);
    t_writer.writeFile(string("Comparison"), variables);
...
/* difmap with key -------------------------------------------------------*/
/*
difmap_wkey::difmap_wkey(std::vector<indexed_points*> datasets1, indexed_points *dataset2,
                            int var_ds1, int var_ds2,
                            ofstream &outfile, string outprefix)
    : difmap(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
string difmap_wkey::compare(){
    ...
    CImg<unsigned char> imgOut( ds1_ip->get_dim()[0] + border_width + key_width,
                                ds1_ip->get_dim()[1],
                                ds1_ip->get_dim()[2],
                                3, 0 );
               ...
               //TODO: loop over every dep_var
							 ...
							 if( this ->restrictToBoxRange 													 &&
									...
									this ->range ->hasPoint( p2.vals[0],p2.vals[1],0.0 				) )
								{
										...
										imgOut(x, ds1_ip->get_dim()[1] - y - 1, z, 0) = col.r;
										imgOut(x, ds1_ip->get_dim()[1] - y - 1, z, 1) = col.g;
										imgOut(x, ds1_ip->get_dim()[1] - y - 1, z, 2) = col.b;
								}
								else if( ! this ->restrictToBoxRange )
								{
									std::vector<int> loc;
    ...
    //Write border
    int border_start = ds1_ip->get_dim()[0];
    ...
    //Write map key
    int x_start = ds1_ip->get_dim()[0] + border_width;

    long double k_height = ds1_ip->get_dim()[1];
...
/* color map -------------------------------------------------------------*/
/*
colmap::colmap(std::vector<indexed_points*> datasets1, indexed_points *dataset2,
                            int var_ds1, int var_ds2,
                            ofstream &outfile, string outprefix)
    : map(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
string colmap::compare(){
    ...
    for(int j=1; j<=2; j++){
        ...
        if(j == 1){
            var_num = var_ds1;
            indep_vars = ds1_ip->get_num_indep_vars();
        }
        else if(j == 2){
            var_num = var_ds2;
            indep_vars = ds2_ip->get_num_indep_vars();
        }
        ...
        for(int indep_var_num=1; indep_var_num <= indep_vars; indep_var_num++){
            variables.push_back(string(itoa(indep_var_num)));
        }
        variables.push_back("Vals");

        if(j == 1)
            t_writer.addOrderedPartition(string("Partition 1"), ds1_ip, var_ds1);
        else if(j == 2)
            t_writer.addOrderedPartition(string("Partition 1"), ds2_ip, var_ds2);
...
long double colmap::get_max_value(int ds, int var){
    ...
    for(int i=0; i<size; i++){
        ...
        if(ds == 1){
            if(dataset1->get_point(i).vals[var] > max){
                max = dataset1->get_point(i).vals[var];
            }
        }

        if(ds == 2){
            if(dataset2->get_point(i).vals[var] > max){
                max = dataset2->get_point(i).vals[var];
            }
        }
    }
...
long double colmap::get_min_value(int ds, int var){
    ...
    for(int i=0; i<size; i++){
        ...
        if(ds == 1){
            if(dataset1->get_point(i).vals[var] < min){
                min = dataset1->get_point(i).vals[var];
            }
        }

        if(ds == 2){
            if(dataset2->get_point(i).vals[var] < min){
                min = dataset2->get_point(i).vals[var];
            }
        }
...
/*
scorco::scorco(std::vector<points*> datasets1, points *dataset2, int var_ds1, int var_ds2,
                ofstream &outfile, string outprefix)
                : metric(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
...
long double scorco::compare(){
    ...
    //Check that number of points matches
    if(!check_card_match())
        return -std::numeric_limits<long double>::infinity();
    ...
    for(int i=0; i<dataset1->get_num_points(); i++)
    {
						...
						this ->range ->hasPoint( p2.vals[0],p2.vals[1],0.0 				) )
				{
						...
						if( !isnan(f1) && !isnan(f2) ){
								...
								//Calculate the sum of dataset1
								sum_f1 += f1;
								//Calculate the square sum of dataset1
								sum_sq_f1 += pow(f1, 2);

								//Calculate the sum of dataset2
								sum_f2 += f2;
								//Calculate the square sume of dataset2
								sum_sq_f2 += pow(f2, 2);
    ...
    //Calculate sample correlation coefficient with calculated values
...
/*
modef::modef(std::vector<points*> datasets1, points *dataset2, int var_ds1, int var_ds2,
                ofstream &outfile, string outprefix)
                : metric(datasets1, dataset2, var_ds1, var_ds2, outfile, outprefix){
