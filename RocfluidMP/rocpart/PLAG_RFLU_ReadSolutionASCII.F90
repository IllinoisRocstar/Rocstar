...
! ******************************************************************************
!
! $Id: PLAG_RFLU_ReadSolutionASCII.F90,v 1.10 2008/12/06 08:44:35 mtcampbe Exp $
!
...
! ******************************************************************************
  ...
  RCSIdentString = '$RCSfile: PLAG_RFLU_ReadSolutionASCII.F90,v $ $Revision: 1.10 $'
  ...
  IF ( global%myProcid == MASTERPROC .AND. &
    ...
    WRITE(STDOUT,'(A,1X,A)') SOLVER_NAME,'Reading ASCII particle file...'
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle x-momentum' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_XMOM,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle y-momentum' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_YMOM,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle z-momentum' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_ZMOM,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle energy' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_ENER,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle x-location' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_XPOS,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle y-location' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_YPOS,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle z-location' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_ZPOS,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle vapor energy' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pCv(CV_PLAG_ENERVAPOR,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle mass' )
        ...
        READ(iFile,'(5(E23.16))') (pCv(iMass,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle superloading' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(E23.16))') (pArv(ARV_PLAG_SPLOAD,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle initial identifier' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(I8))') (pAiv(AIV_PLAG_PIDINI,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle initial region' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(I8))') (pAiv(AIV_PLAG_REGINI,j),j=1,pPlag%nPcls)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Particle cell' )
        ...
        iVars = iVars + 1
        READ(iFile,'(5(I8))') (pAiv(AIV_PLAG_ICELLS,j),j=1,pPlag%nPcls)        
        ...
        DO iPatch = 1,pGrid%nPatches
          ...
          IF ( (pPatch%bcType >= BC_INJECTION .AND. pPatch%bcType <= BC_INJECTION + BC_RANGE) .OR. &
            ...
            DO ifl = 1,pPatch%nBFaces
              READ(iFile,'(2(E23.16))') pTilePlag%cv(CV_TILE_MOMNRM,ifl), & 
                                        pTilePlag%cv(CV_TILE_ENER  ,ifl)
            END DO ! ifl 
            
            DO iCont = 1,nCont
              iMass = pTilePlag%cvTileMass(iCont)
              READ(iFile,'(5(E23.16))') (pTilePlag%cv(iMass,ifl), &
                                         ifl=1,pPatch%nBFaces)
            END DO ! iCont            
            
            DO ifl = 1,pPatch%nBFaces
              READ(iFile,'(3(E23.16))') pTilePlag%dv(DV_TILE_COUNTDOWN,ifl), & 
...
! ==============================================================================
  ...
  IF ( global%myProcid == MASTERPROC .AND. &
       global%verbLevel > VERBOSE_NONE ) THEN   
    WRITE(STDOUT,'(A,1X,A)') SOLVER_NAME,'Reading ASCII particle file done.'
  END IF ! global%verbLevel
...
!
! $Log: PLAG_RFLU_ReadSolutionASCII.F90,v $
! Revision 1.10  2008/12/06 08:44:35  mtcampbe
! Updated license.
!
! Revision 1.9  2008/11/19 22:17:48  mtcampbe
! Added Illinois Open Source License/Copyright
!
! Revision 1.8  2007/03/31 23:56:17  haselbac
! Removed superfluous close parentheses
!
! Revision 1.7  2006/09/18 20:37:02  fnajjar
! Activated tile datastructure for inflow bc
!
! Revision 1.6  2005/01/21 17:23:10  fnajjar
! Included vapor energy in IO capability
!
! Revision 1.5  2004/06/16 23:03:09  fnajjar
! Renamed DV_TILE_TIMEFCTR to DV_TILE_COUNTDOWN for CRE kernel
!
! Revision 1.4  2004/06/16 20:01:19  haselbac
! Added use of ModBuildFileNames, cosmetics
!
! Revision 1.3  2004/05/17 15:05:16  haselbac
! Bug fix: Only update iVars for first constituent
!
! Revision 1.2  2004/03/05 23:19:16  haselbac
! Added reading of additional patch data and modified reading of constituents
!
! Revision 1.1  2004/02/26 21:00:50  haselbac                                  
! Initial revision                                                             
!
! ******************************************************************************
