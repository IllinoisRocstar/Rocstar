...
!******************************************************************************
!
! $Id: PEUL_ReadBcInjectSection.F90,v 1.4 2008/12/06 08:44:39 mtcampbe Exp $
!
...
!******************************************************************************
  ...
  RCSIdentString = &
    '$RCSfile: PEUL_ReadBcInjectSection.F90,v $ $Revision: 1.4 $'
...
! define keys
  ...
  keys(iKeyFrac) = 'FRAC_'
  CALL MakeNumberedKeys(keys,iKeyFrac0+1,'FRAC',1,NPEUL_KEYS,1)

  nKeys = iKeyFrac0 + NPEUL_KEYS
...
! ----- check if appropriate values specified
        IF ( patch%peul%distrib == BCDAT_CONSTANT ) THEN
          IF (.NOT. defined(iKeyFrac)) THEN
            alldef = .TRUE.
            DO ipt = 1, regions(iReg)%peulInput%nPtypes
              alldef = alldef .AND. defined(iKeyFrac0+ipt)
            END DO ! ipt
...
! ----- distribution from external source / constant value
        ...
        ELSE
          ...
          IF (ASSOCIATED(patch%peul%vals)) THEN
            IF (defined(iKeyFrac)) patch%peul%vals(:,:) = vals(iKeyFrac)
            DO ipt = 1, patch%peul%nData
              IF (defined(iKeyFrac0+ipt)) patch%peul%vals(ipt,:) = &
                vals(iKeyFrac0+ipt)
            END DO ! ipt
          END IF
...
!
! $Log: PEUL_ReadBcInjectSection.F90,v $
