...
! ******************************************************************************
!
! $Id: RFLO_ModMoveGridNconform.F90,v 1.37 2009/08/27 14:04:50 mtcampbe Exp $
!
...
! ******************************************************************************  
  ...
  CHARACTER(CHRLEN) :: RCSIdentString = & 
    '$RCSfile: RFLO_ModMoveGridNconform.F90,v $ $Revision: 1.37 $'        
...
! ... local variables
  ...
  INTEGER :: ibeg, iend, jbeg, jend, kbeg, kend, nCorns
        ...
        IF ((bcType>=BC_INFLOW     .AND. bcType<=BC_INFLOW    +BC_RANGE) .OR. &
          ...
          DO ic = 1,grid%nCorns(iReg)
            IF (ijkNode(1)==grid%ijkCorn(ic,iReg) .OR. &
                ...
                ijkNode(4)==grid%ijkCorn(ic,iReg)) grid%nghbor(3,1:6,ic)= 2
          ENDDO
        ENDIF    ! bc_external
      ENDDO      ! iPatch
        ...
        IF (patch%bcMotion == BC_EXTERNAL) THEN
          ...
          DO ic = 1,grid%nCorns(iReg)
            IF (ijkNode(1)==grid%ijkCorn(ic,iReg) .OR. &
                ijkNode(2)==grid%ijkCorn(ic,iReg) .OR. &
                ijkNode(3)==grid%ijkCorn(ic,iReg) .OR. &
                ijkNode(4)==grid%ijkCorn(ic,iReg)) grid%nghbor(3,1:6,ic)= 0
          ENDDO
        ENDIF    ! bc_external
      ENDDO      ! iPatch
...
! ... local variables
  INTEGER :: iLev, interior, nco(6), nReg(6), ijkCorn
  INTEGER :: ipnbeg, ipnend, jpnbeg, jpnend, kpnbeg, kpnend, iNOff, ijNOff
...
! ... local variables
  ...
  INTEGER :: indBeg, indEnd, ijkN, ijkN1, ijkNBeg, ijkNEnd, iNOff, ijNOff
  INTEGER :: lbound, intb, inte, iEdge, interType
    ...
    DO ipEdge = 1,4
      ...
      IF (lbound==1 .OR. lbound==2) THEN
        IF (ipEdge==1) THEN
          ...
          DO ic = 1,grid%nCorns(region%iRegionGlobal)
            IF (ic==ijkNBeg) intb = grid%nghbor(3,1,ic) 
            IF (ic==ijkNEnd) inte = grid%nghbor(3,1,ic)
          ENDDO

          IF (((intb/=0 .OR. inte/=0) .AND. (interType==EDGE_INTERACT_FULL .AND. &
                region%levels(iLev)%edgeCells(iEdge)%interact .EQV. .true.)) .OR.&
...
!                CALL RFLO_MgFrameEdges( regions(iReg),grid%edgeMoved, &
!                                      gridOld%xyzOld,grid%xyz )
!                CALL RFLO_MgFrameBndDeformation( regions(iReg), &
!                                      grid%boundMoved, &
!                                      grid%edgeMoved,grid%arcLen12, &
!                                      grid%arcLen34,grid%arcLen56, &
!                                      gridOld%xyzOld,grid%xyz )
...
!
! $Log: RFLO_ModMoveGridNconform.F90,v $
! Revision 1.37  2009/08/27 14:04:50  mtcampbe
! Updated to enable burning motion with symmetry boundaries and enhanced
! burnout code.
!
! Revision 1.36  2008/12/06 08:44:17  mtcampbe
! Updated license.
!
! Revision 1.35  2008/11/19 22:17:28  mtcampbe
! Added Illinois Open Source License/Copyright
!
! Revision 1.34  2006/03/18 11:02:38  wasistho
! screen printed global skewness and minvol
!
! Revision 1.33  2006/03/05 22:27:41  wasistho
! fixed syntax error
!
! Revision 1.32  2006/03/05 21:52:00  wasistho
! changed computational space coordinates to be based on initial grid
!
! Revision 1.31  2006/02/11 03:53:18  wasistho
! made some routines public
!
! Revision 1.30  2006/01/28 22:52:34  wasistho
! fixed iEdgeGlo
!
! Revision 1.29  2005/11/16 22:52:27  wasistho
! update screen print moveGridFOMS
!
! Revision 1.28  2005/10/28 07:41:05  wasistho
! modified FORMAT 1000
!
! Revision 1.27  2005/10/27 19:19:49  wasistho
! modified screen print
!
! Revision 1.26  2005/10/27 05:57:11  wasistho
! added MoveGridFoms
!
! Revision 1.25  2005/08/18 19:51:12  wasistho
! added user define nPass in mgFrameInterface
!
! Revision 1.24  2005/07/10 21:13:04  wasistho
! global alloc dist in SrchNeighbors and CorrectNeighbors, and added pointer grid in mgFrameMoveCorners
!
! Revision 1.23  2005/06/30 19:10:27  wasistho
! made official last added conditions in mgFrameEdges
!
! Revision 1.22  2005/06/29 08:38:56  wasistho
! changed RestoreExternal
!
! Revision 1.21  2005/06/29 02:37:02  wasistho
! activated itype=2
!
! Revision 1.20  2005/06/27 19:23:04  wasistho
! only ipass=1 in mgFrameInterfaces
!
! Revision 1.19  2005/06/27 01:00:53  wasistho
! stored tolerance in tol
!
! Revision 1.18  2005/06/27 00:36:52  wasistho
! change tolerance in mgFrameSrchNeighbors from 1.e-20 to 1.e-5*edgeLen
!
! Revision 1.17  2005/06/26 06:25:40  wasistho
! nReg==72 to nReg==71
!
! Revision 1.16  2005/06/26 06:12:00  wasistho
! adding more reegions for titan check in mgFrameSrchCorners
!
! Revision 1.15  2005/06/26 05:39:44  wasistho
! bugs fixed in mgFrameCornPoints and mgFrameSrchNeighbors
!
! Revision 1.14  2005/06/25 08:12:26  wasistho
! bug fixed in receiving rvar in mgFrameBCast
!
! Revision 1.13  2005/06/25 06:16:15  wasistho
! swap ENDDO and ENDIF in mgframeBroadCast
!
! Revision 1.12  2005/06/25 03:13:57  wasistho
! enabled nRegions /= nProcs in type 2 gridmotion
!
! Revision 1.11  2005/06/19 12:36:07  wasistho
! update in mgFrameInterfaces
!
! Revision 1.10  2005/06/15 19:20:40  wasistho
! simplified mgFrameInterfaces and copied RFLO_ModMoveGridNconform to RFLO_ModMoveGridFrame
!
! Revision 1.9  2005/06/14 11:20:46  wasistho
! set itype to 1
!
! Revision 1.8  2005/06/13 21:47:31  wasistho
! changed patch%bcCoupled to patch%bcMotion
!
! Revision 1.7  2005/06/12 10:56:33  wasistho
! uncommented second/end TFI procedure
!
! Revision 1.6  2005/06/12 10:12:08  wasistho
! commented second/end TFI procedure
!
! Revision 1.5  2005/06/12 08:26:20  wasistho
! set initial ds=0 in mgFrameEdge
!
