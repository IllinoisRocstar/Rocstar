...
!******************************************************************************
!
! Purpose: Reads in information related to the interaction HeatTransferNonBurn
!
! Description: none.
!
...
!******************************************************************************
!
! $Id: INRT_ReadHeatTransferNonBurn.F90,v 1.3 2008/12/06 08:44:32 mtcampbe Exp $
!
...
SUBROUTINE INRT_ReadHeatTransferNonBurn( regions )
  ...
  USE INRT_ModInterfaces, ONLY : INRT_SetActiveness,INRT_SetPermission, &
                                 ...
                                 INRT_DefineHeatTransferNonBurn
  IMPLICIT NONE
...
!******************************************************************************
  ...
  RCSIdentString = &
    '$RCSfile: INRT_ReadHeatTransferNonBurn.F90,v $ $Revision: 1.3 $'

  global => regions(1)%global

  CALL RegisterFunction( global,'INRT_ReadHeatTransferNonBurn',&
...
! - Define interaction (using any relevant information from input deck)
    ...
    CALL INRT_DefineHeatTransferNonBurn(regions(iReg))

! - Check for switches

    inrt%switches(INRT_SWI_HTRANSNB_MODEL) = INRT_HTRANSNB_MODEL_DEFAULT
    ...
    IF (defined(iKeyModel)) THEN
      ...
      CASE (1)
        inrt%switches(INRT_SWI_HTRANSNB_MODEL) = INRT_HTRANSNB_MODEL_STOKES

      CASE (2)
        inrt%switches(INRT_SWI_HTRANSNB_MODEL) = INRT_HTRANSNB_MODEL_RM
...
!
! $Log: INRT_ReadHeatTransferNonBurn.F90,v $
...
!
! Revision 1.1  2004/12/01 21:56:36  fnajjar
...
!
! Revision 1.5  2004/03/02 21:49:22  jferry
