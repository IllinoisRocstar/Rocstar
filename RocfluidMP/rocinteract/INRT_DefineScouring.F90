...
!******************************************************************************
!
! Purpose: Defines the interaction Scouring
!
! Description: none.
!
...
!******************************************************************************
!
! $Id: INRT_DefineScouring.F90,v 1.5 2008/12/06 08:44:31 mtcampbe Exp $
!
...
! ... loop variables
  INTEGER :: iPlag,iPeul,iEdge

! ... local variables
  INTEGER, PARAMETER :: NEDGES_MAX = 20

  CHARACTER(CHRLEN) :: RCSIdentString

  INTEGER :: nEdges,nPlag,nPeul
  INTEGER :: matIndPlag,matIndPeul
  INTEGER :: iPeulArr(NEDGES_MAX),iPlagArr(NEDGES_MAX)
...
!******************************************************************************
  ...
  RCSIdentString = '$RCSfile: INRT_DefineScouring.F90,v $ $Revision: 1.5 $'
...
! loop over smoke types iPeul looking for corresponding iPlag
  ...
  DO iPeul = 1,nPeul
    ...
    DO iPlag = 1,nPlag
      ...
      IF (matIndPlag == matIndPeul) THEN
        ...
        iPeulArr(iEdge) = iPeul
        iPlagArr(iEdge) = iPlag
  ...
  CALL INRT_AllocateAuxillary(global,inrt,nEdges, &
    INRT_SWI_SCOURING_TOTAL,INRT_DAT_SCOURING_TOTAL0 + nEdges)
...
! define Edges
  ...
  DO iEdge = 1,nEdges
    ...
    edge%iNode(1) = input%indPeul0 + iPeulArr(iEdge)
    edge%iNode(2) = input%indPlag0 + iPlagArr(iEdge)
...
!
! Revision 1.3  2006/02/15 20:39:49  wasistho
! put plag within ifdef
!
! Revision 1.2  2006/02/15 20:17:57  wasistho
! put peul within ifdef
!
! Revision 1.1  2004/12/01 21:56:24  fnajjar
