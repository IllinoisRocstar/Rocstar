...
!******************************************************************************
!
! $Id: RFLO_ReadBcOutflowSection.F90,v 1.7 2008/12/06 08:44:07 mtcampbe Exp $
!
...
! ----- check if switch defined
        ...
        ELSE
          ...
          __LINE__,'(outflow type).' )
        ENDIF

        IF (defined(3).eqv..true.) THEN
          IF (vals(3) < 0.1) &
            patch%mixt%switches(BCSWI_OUTFLOW_MODEL) = BCOPT_DEFAULT
          IF (vals(3) > 0.1) &
...
! ----- check if appropriate values specified
        ...
        IF (patch%mixt%switches(BCSWI_OUTFLOW_MODEL) == BCOPT_MODEL1) THEN
          IF (.NOT. (defined(4).eqv..true.)) CALL ErrorStop( global,ERR_BCVAL_MISSING, &
            __LINE__,'BC_OUTFLOW: specify NRCOEF for model 1' )
        ENDIF
...
! ----- distribution from external source / constant value
        ...
        IF (patch%mixt%switches(BCSWI_OUTFLOW_MODEL) == BCOPT_MODEL1) THEN
          IF (vals(4) < 0._RFREAL) CALL ErrorStop( global,ERR_ILLEGAL_VALUE, &
                          __LINE__,'BC_OUTFLOW: NRCOEF cannot be negative' )
          patch%mixt%vals(BCDAT_OUTFLOW_NRCOEF,:) = vals(4)
        ENDIF
...
!
! $Log: RFLO_ReadBcOutflowSection.F90,v $
...
!
! Revision 1.4  2006/08/19 15:38:19  mparmar
! Renamed patch variables
!
! Revision 1.3  2005/05/13 08:07:15  wasistho
! added additional check for NRCOEF value
!
! Revision 1.2  2005/05/13 06:07:28  wasistho
...
!
! Revision 1.3  2003/02/11 22:30:21  jferry
! Re-worked BC and TBC input routines to add multi-physics capability
!
! Revision 1.2  2003/01/10 17:58:43  jblazek
...
!
! Revision 1.7  2002/03/30 00:50:49  jblazek
