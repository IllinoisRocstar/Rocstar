...
!******************************************************************************
!
! Purpose: read in real or integer data from a file (grid or solution).
!
! Description: file contains the following subroutines:
!
! - RFLO_ReadDataFileReal = real variable
! - RFLO_ReadDataFileInt  = integer variable
!
! Input: fileId  = I/O channel
!        form    = type of file (ASCII, binary)
!        nDim1/2 = 1st and 2nd dimension of var.
!
! Output: var  = real variable to read
...
!******************************************************************************
!
! $Id: RFLO_ReadDataFile.F90,v 1.3 2008/12/06 08:44:07 mtcampbe Exp $
!
...
! read
  ...
  IF (form == FORMAT_ASCII) THEN
    READ(fileId,*,err=10,end=10) ((ivar(i1,i2), i2=1,nDim2), i1=1,nDim1)
...
! read
  ...
  IF (form == FORMAT_ASCII) THEN
    READ(fileId,*,err=10,end=10) ((var(i1,i2), i2=1,nDim2), i1=1,nDim1)
  ELSE IF (form == FORMAT_BINARY) THEN
    READ(fileId,err=10,end=10)   ((var(i1,i2), i2=1,nDim2), i1=1,nDim1)
  ELSE
    CALL ErrorStop( global,ERR_UNKNOWN_FORMAT,&
