...
!******************************************************************************
!
! Purpose: Perform cell to cell uniform filtering in I-direction.
!
...
!******************************************************************************
!
! $Id: TURB_floLesUniFiltCCI.F90,v 1.5 2008/12/06 08:44:43 mtcampbe Exp $
!
! Copyright: (c) 2001 by the University of Illinois
!
!******************************************************************************

SUBROUTINE TURB_FloLesUniFiltCCI( global,nDum,ibeg,iend,jbeg,jend,kbeg,kend, &
...
!******************************************************************************
  ...
  RCSIdentString = '$RCSfile: TURB_floLesUniFiltCCI.F90,v $'

  CALL RegisterFunction( global,'TURB_FloLesUniFiltCCI',&
...
! ----- perform integration
        ...
        DO i=ibeg+1,iend-1
          ...
          DO l=idBeg,idEnd
            filtVar(l,ijkC)=fact1(nDel(DIRI))*(fVar(l,ijkC-1)+fVar(l,ijkC+1))+ &
                            fact2(nDel(DIRI))*fVar(l,ijkC)
          END DO ! l
        ENDDO    ! i
...
! ----- perform integration at the edges
        ...
        DO l=idBeg,idEnd
          ...
          filtVar(l,ijkCb)=fact1(nDel(DIRI))*(tmpb(l,1)+fVar(l,ijkCb+1))+ &
                           fact2(nDel(DIRI))*fVar(l,ijkCb)
          filtVar(l,ijkCe)=fact1(nDel(DIRI))*(tmpe(l,1)+fVar(l,ijkCe-1))+ &
                           fact2(nDel(DIRI))*fVar(l,ijkCe)
        ENDDO   ! l
      ENDDO     ! j
    ENDDO       ! k
...
! ----- perform integration
        ...
        DO i=ibeg+2,iend-2           
          ijkC = IndIJK(i     ,j     ,k     ,iCOff,ijCOff)
          DO l=idBeg,idEnd
            filtVar(l,ijkC)=fact1(nDel(DIRI))*(fVar(l,ijkC-2)+fVar(l,ijkC+2))+ &
                            fact2(nDel(DIRI))*(fVar(l,ijkC+1)+fVar(l,ijkC)+ &
                                               fVar(l,ijkC-1))
          ENDDO ! l
        ENDDO   ! i
...
! ----- perform integration at the edges
        ...
        DO l=idBeg,idEnd
          tmpb(l,1)=2._RFREAL*fVar(l,ijkCb)-fVar(l,ijkCb+1)
          tmpe(l,1)=2._RFREAL*fVar(l,ijkCe)-fVar(l,ijkCe-1)
          filtVar(l,ijkCb1)=fact1(nDel(DIRI))*(tmpb(l,1)+fVar(l,ijkCb1+2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkCb1+1)+fVar(l,ijkCb1)+ &
                                              fVar(l,ijkCb1-1))
          filtVar(l,ijkCe1)=fact1(nDel(DIRI))*(tmpe(l,1)+fVar(l,ijkCe1-2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkCe1+1)+fVar(l,ijkCe1)+ &
                                              fVar(l,ijkCe1-1))
        ENDDO   ! l
        ...
        DO l=idBeg,idEnd
          ...
          filtVar(l,ijkCb)=fact1(nDel(DIRI))*(tmpb(l,2)+fVar(l,ijkCb+2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkCb+1)+fVar(l,ijkCb)+ &
                                              tmpb(l,1))
          filtVar(l,ijkCe)=fact1(nDel(DIRI))*(tmpe(l,2)+fVar(l,ijkCe-2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkCe-1)+fVar(l,ijkCe)+ &
                                              tmpe(l,1))
        ENDDO   ! l
      ENDDO     ! j
    ENDDO       ! k
...
!
! $Log: TURB_floLesUniFiltCCI.F90,v $
...
!
! Revision 1.3  2004/06/03 02:12:45  wasistho
...
!
! Revision 1.3  2003/05/16 05:44:03  wasistho
