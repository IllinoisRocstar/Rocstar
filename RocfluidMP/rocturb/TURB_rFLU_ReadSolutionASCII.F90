...
! ******************************************************************************
!
! Purpose: Read file for turbulence quantities in ASCII ROCFLU format.
!
! Description: None.
!
...
! ******************************************************************************
!
! $Id: TURB_rFLU_ReadSolutionASCII.F90,v 1.6 2009/08/26 12:28:53 mtcampbe Exp $
!
! Copyright: (c) 2004 by the University of Illinois
!
! ******************************************************************************

SUBROUTINE TURB_RFLU_ReadSolutionASCII( region ) ! PUBLIC
...
! ******************************************************************************
  ...
  RCSIdentString = '$RCSfile: TURB_rFLU_ReadSolutionASCII.F90,v $ $Revision: 1.6 $'
  ...
  IF ( global%myProcid == MASTERPROC .AND. &
    ...
    WRITE(STDOUT,'(A,1X,A)') SOLVER_NAME,'Reading ASCII turbulence file...'
...
! -----------------------------------------------------------------------------
  ...
  READ(iFile,'(E23.16)') global%esg1Sum
  ...
  READ(iFile,'(E23.16)') global%esg4Sum
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Eddy viscosity' )
        ...
        IF (region%turbInput%modelClass == MODEL_LES) THEN
          ...
          iVars = iVars + 1
          READ(iFile,'(5(E23.16))') (tv(TV_MIXT_MUET,j),j=1,grid%nCellsTot)
        ...
        ELSEIF (region%turbInput%modelClass == MODEL_RANS) THEN
          ...
          iVars = iVars + 1
          READ(iFile,'(5(E23.16))') (tcv(CV_SA_NUTIL,j),j=1,grid%nCellsTot)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# Total vorticity' ) 
        ...
        iVars = iVars + 1      
        READ(iFile,'(5(E23.16))') (vort(XCOORD,j),j=1,grid%nCellsTot)
...
! ------------------------------------------------------------------------------
      ...
      CASE ( '# RANS length scale' ) 
        ...
        iVars = iVars + 1      
        READ(iFile,'(5(E23.16))') (lens(j),j=1,grid%nCellsTot)
...
! ==============================================================================
  ...
  IF ( global%myProcid == MASTERPROC .AND. & 
       global%verbLevel > VERBOSE_NONE ) THEN   
    WRITE(STDOUT,'(A,1X,A)') SOLVER_NAME,'Reading ASCII turbulence file done.'
  END IF ! global%verbLevel
...
!
! $Log: TURB_rFLU_ReadSolutionASCII.F90,v $
...
!
! Revision 1.3  2006/01/12 09:44:09  wasistho
! copy to postv
!
! Revision 1.2  2004/06/16 20:01:30  haselbac
