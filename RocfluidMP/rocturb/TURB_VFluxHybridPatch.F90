...
!******************************************************************************
!
! Purpose: Compute at current boundary patch total viscous fluxes of which 
!          turbulent stress contribution is based on stress model as opposed 
!          to eddy viscosity model.
!
! Description: this routine works in the same way as TURB_vFluxHybrid
...
!******************************************************************************
!
! $Id: TURB_VFluxHybridPatch.F90,v 1.14 2008/12/06 08:44:42 mtcampbe Exp $
!
...
!******************************************************************************
  ...
  RCSIdentString = '$RCSfile: TURB_VFluxHybridPatch.F90,v $'
...
!endpert
          ...
          tx  = grad(XCOORD,GRBF_MIXT_TEMP,ijkNBG)
          ty  = grad(YCOORD,GRBF_MIXT_TEMP,ijkNBG)
          tz  = grad(ZCOORD,GRBF_MIXT_TEMP,ijkNBG)
#endif
...
!$endcopy
          tij(1,1) = tauij(E11,ijkNBG) - mueT(ijk,ijkNBG)*sij(1,1)
          tij(1,2) = tauij(E12,ijkNBG) - mueT(ijk,ijkNBG)*sij(1,2)
          tij(1,3) = tauij(E13,ijkNBG) - mueT(ijk,ijkNBG)*sij(1,3)

          tij(2,1) = tauij(E12,ijkNBG) - mueT(ijk,ijkNBG)*sij(2,1)
          tij(2,2) = tauij(E22,ijkNBG) - mueT(ijk,ijkNBG)*sij(2,2)
          tij(2,3) = tauij(E23,ijkNBG) - mueT(ijk,ijkNBG)*sij(2,3)

          tij(3,1) = tauij(E13,ijkNBG) - mueT(ijk,ijkNBG)*sij(3,1)
          tij(3,2) = tauij(E23,ijkNBG) - mueT(ijk,ijkNBG)*sij(3,2)
          tij(3,3) = tauij(E33,ijkNBG) - mueT(ijk,ijkNBG)*sij(3,3)

          taukk = tij(1,1)+tij(2,2)+tij(3,3)
          taukk = MAX( 0._RFREAL,taukk )
          trace(ijkCB0) = trace(ijkCB0) + taukk

          tfd(1)   = tij(1,1)*sf(1)+tij(1,2)*sf(2)+tij(1,3)*sf(3)
          tfd(2)   = tij(2,1)*sf(1)+tij(2,2)*sf(2)+tij(2,3)*sf(3)
          tfd(3)   = tij(3,1)*sf(1)+tij(3,2)*sf(2)+tij(3,3)*sf(3)
...
! 
! $Log: TURB_VFluxHybridPatch.F90,v $
! Revision 1.14  2008/12/06 08:44:42  mtcampbe
! Updated license.
!
! Revision 1.13  2008/11/19 22:17:54  mtcampbe
! Added Illinois Open Source License/Copyright
!
! Revision 1.12  2006/08/19 15:40:39  mparmar
! Removed bf2bg, used GRBF_ and moved region%mixt%bGradFace to patch%mixt%gradFace
!
! Revision 1.11  2006/03/13 03:44:28  wasistho
! defined aero coeffs
!
! Revision 1.10  2004/10/22 23:20:44  wasistho
! collect esg1 and esg2+3 into statistics
!
! Revision 1.9  2004/08/04 22:07:07  wasistho
! bugfixed: ac0 and ac1 are common to flo and flu
!
! Revision 1.8  2004/08/02 21:06:24  wasistho
! moved averaging coeff local variables to within ifdef RFLO
!
! Revision 1.7  2004/08/02 19:34:02  wasistho
! changed grid%avgCo to grid%c2fCo
!
! Revision 1.6  2004/07/31 00:53:52  wasistho
! replaced cell2face midpoint by linear averaging
!
! Revision 1.5  2004/05/29 03:38:41  wasistho
...
!
! Revision 1.6  2004/02/26 21:22:36  wasistho
! changed turb%esg.. to global%esg..
!
! Revision 1.5  2004/01/21 03:42:29  wasistho
! get rid of grad(T)*n = 0 for adiabatic wall
!
! Revision 1.4  2003/08/29 01:43:04  wasistho
! Added TARGET attribute to region variable, since pointers are cached into it
!
! Revision 1.3  2003/06/05 19:20:51  wasistho
! adiabatic switch
!
