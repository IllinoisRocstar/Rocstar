...
!******************************************************************************
!
! Purpose: update RaNS variables in dummy cells at no-slip patch
!
! Description: none.
!
...
!******************************************************************************
!
! $Id: TURB_floRansBcondNoslipWall.F90,v 1.5 2008/12/06 08:44:44 mtcampbe Exp $
!
...
! loop over all cells of a patch
      ...
      DO k=kbeg,kend
        DO j=jbeg,jend
          DO i=ibeg,iend
            ...
            densRat = cv(CV_MIXT_DENS,ijkD)/cv(CV_MIXT_DENS,ijkC1)
            DO l=1,nCv
              tcv(l,ijkD) = - tcv(l,ijkC1)*densRat
            ENDDO
          ENDDO  ! i
        ENDDO    ! j
      ENDDO      ! k
  ...
  ELSE
    ...
    CALL ErrorStop( region%global,ERR_TURB_RANSINPUT,__LINE__, &
                   'noslip wall bc is not ready yet for RaNS model selected' )
  ENDIF
...
!
! $Log: TURB_floRansBcondNoslipWall.F90,v $
...
!
! Revision 1.3  2005/03/09 06:37:04  wasistho
! incorporated HDESSA
!
...
!
! Revision 1.2  2004/01/21 03:45:39  wasistho
! set noslip bc for tilde[nu] i.o. rho*tilde[nu]
!
