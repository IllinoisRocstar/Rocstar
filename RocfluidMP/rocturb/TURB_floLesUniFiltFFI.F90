...
!******************************************************************************
!
! Purpose: Perform face to face uniform filtering in I-direction.
!
...
!******************************************************************************
!
! $Id: TURB_floLesUniFiltFFI.F90,v 1.4 2008/12/06 08:44:44 mtcampbe Exp $
!
...
!******************************************************************************
  ...
  RCSIdentString = '$RCSfile: TURB_floLesUniFiltFFI.F90,v $'

  CALL RegisterFunction( global,'TURB_FloLesUniFiltFFI',&
...
! ----- perform integration
        ...
        DO i=ibeg,iend
          ...
          DO l=idBeg,idEnd
            filtVar(l,ijkN)=fact1(nDel(DIRI))*(fVar(l,ijkN-1)+fVar(l,ijkN+1))+ &
                            fact2(nDel(DIRI))*fVar(l,ijkN)
          END DO ! l
        ENDDO    ! i
...
! ----- perform integration
        ...
        DO i=ibeg+1,iend-1           
          ijkN = IndIJK(i     ,j     ,k     ,iNOff,ijNOff)
          DO l=idBeg,idEnd
            filtVar(l,ijkN)=fact1(nDel(DIRI))*(fVar(l,ijkN-2)+fVar(l,ijkN+2))+ &
                            fact2(nDel(DIRI))*(fVar(l,ijkN+1)+fVar(l,ijkN)+ &
                                               fVar(l,ijkN-1))
          ENDDO ! l
        ENDDO   ! i
...
! ----- perform integration at the edges
        ...
        DO l=idBeg,idEnd
          ...
          filtVar(l,ijkNb)=fact1(nDel(DIRI))*(tmpb(l)+fVar(l,ijkNb+2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkNb+1)+fVar(l,ijkNb)+ &
                                              fVar(l,ijkNb-1))
          filtVar(l,ijkNe)=fact1(nDel(DIRI))*(tmpe(l)+fVar(l,ijkNe-2))+ &
                           fact2(nDel(DIRI))*(fVar(l,ijkNe+1)+fVar(l,ijkNe)+ &
                                              fVar(l,ijkNe-1))
        ENDDO   ! l
      ENDDO     ! j
    ENDDO       ! k
...
!
! $Log: TURB_floLesUniFiltFFI.F90,v $
