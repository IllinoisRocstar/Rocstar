...
!******************************************************************************
!
! Purpose: read in radiation solution.
!
...
!******************************************************************************
!
! $Id: RADI_rFLO_ReadSolution.F90,v 1.3 2008/12/06 08:44:38 mtcampbe Exp $
!
! Copyright: (c) 2001 by the University of Illinois
!
!******************************************************************************

SUBROUTINE RADI_RFLO_ReadSolution( regions ) ! PUBLIC
...
! ... local variables
  ...
  INTEGER :: nDimC, nRvar, nOutSol, nAng, nSolComp, errorFlag
  INTEGER, ALLOCATABLE :: ivar(:,:)

  REAL(RFREAL), POINTER     :: rcv(:,:)
  REAL(RFREAL), ALLOCATABLE :: rvar(:,:), solFile(:,:), dumFile(:,:)
...
!******************************************************************************
  ...
  CALL RegisterFunction( global,'RADI_RFLO_ReadSolution',&
...
! - unsteady flow
    ...
    IF (global%flowType == FLOW_UNSTEADY) THEN
      IF (global%solutFormat == FORMAT_ASCII) THEN
        WRITE(fname,'(A,1PE11.5)') TRIM(global%inDir)//TRIM(global%casename)//'.radia_', &
                                   global%timeStamp
        OPEN(IF_SOLUT,file=fname,form='formatted',status='old',iostat=errorFlag)
      ELSE IF (global%solutFormat == FORMAT_BINARY) THEN
        WRITE(fname,'(A,1PE11.5)') TRIM(global%inDir)//TRIM(global%casename)//'.radib_', &
                                   global%timeStamp
...
! - steady flow
    ...
    ELSE
      IF (global%solutFormat == FORMAT_ASCII) THEN
        WRITE(fname,'(A,I6.6)') TRIM(global%inDir)//TRIM(global%casename)//'.radia_', &
                                global%currentIter
        OPEN(IF_SOLUT,file=fname,form='formatted',status='old',iostat=errorFlag)
      ELSE IF (global%solutFormat == FORMAT_BINARY) THEN
        WRITE(fname,'(A,I6.6)') TRIM(global%inDir)//TRIM(global%casename)//'.radib_', &
                                global%currentIter
        OPEN(IF_SOLUT,file=fname,form='unformatted',status='old',iostat=errorFlag)
      ELSE
        CALL ErrorStop( global,ERR_UNKNOWN_FORMAT,__LINE__ )
...
! - master reads & sends data, others receive them
    ...
    IF (global%myProcid == MASTERPROC) THEN
      ...
      ALLOCATE( dumFile(1,nDimC),stat=errorFlag )
      global%error = errorFlag
      IF (global%error /= 0) CALL ErrorStop( global,ERR_ALLOCATE,__LINE__ )
        ...
        DO k=kdcbeg,kdcend
          DO j=jdcbeg,jdcend
            DO i=idcbeg,idcend
              ...
              rcv(TV_MIXT_MUET,ijk) = solFile(2,n)
            ENDDO
          ENDDO
        ENDDO
      ENDIF    ! radiModel
...
!
! $Log: RADI_rFLO_ReadSolution.F90,v $
