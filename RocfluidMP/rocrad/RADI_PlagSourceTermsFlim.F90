...
!******************************************************************************
!
! Purpose: added radiation source terms to the rhs of PLAG Energy eq.
!
! Description: see formulation of FLD (Howell et al. JCP 184 (2003) 53-78)
!
! Input: region = data of current region,
!
! Output: region%levels%plag%rhs, radiaton source terms added to PLAG rhs
!
! Notes: none.
!
!******************************************************************************
!
! $Id: RADI_PlagSourceTermsFlim.F90,v 1.4 2008/12/06 08:44:37 mtcampbe Exp $
!
...
! get dimensions and pointers -------------------------------------------------
  ...
  rhs   => region%levels(iLev)%plag%rhs
#endif
  rcv   => region%levels(iLev)%radi%cv
!  coef  => region%levels(iLev)%radi%plagCoef
...
!        bTermAlp  = fourStb*dv(DV_PLAG_TEMP,ijkC)**4
!        sTerm = bTermAlp - dv(DV_MIXT_SOUN,ijkC)*rcv(CV_RADI_ENER,ijkC)
!        sTerm = planckAlp*sTerm

!        rhs(CV_PLAG_ENER,ijkC) = rhs(CV_PLAG_ENER,ijkC) - vol(ijkC)*sTerm
...
!
! $Log: RADI_PlagSourceTermsFlim.F90,v $
...
!
! Revision 1.2  2006/02/15 19:38:35  wasistho
